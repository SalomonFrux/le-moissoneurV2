      ...
      try {
        ...
        trendArray = trendArray.map(month => {
          ...
          return {
            ...
            completeness: month.completeness / month.new // Average completeness for the month
          };
        });
        ...
        <div className="bg-white p-2 border rounded shadow">
          ...
          {payload.map((p: any) => (
            <p key={p.name} style={{ color: p.color }}>
              {p.name === 'total' ? 'Total: ' : 
               p.name === 'new' ? 'Nouvelles: ' : 
               p.name === 'completeness' ? 'Complétude: ' : 'Taux: '}
              {p.value.toFixed(1)} {p.name === 'enrichmentRate' || p.name === 'completeness' ? '%' : ''}
            </p>
          ))}
        </div>
      );
